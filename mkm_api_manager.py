...
from util import pretty, response_code_valid
import json
...
def filter_deals(articles, condition_min, min_price, max_price):
    ...
    articles_worth = list(filter(
        lambda article:
            "condition" in article and
            condition_to_num[article["condition"]] >= condition_to_num[condition_min] and
            article["price"] >= min_price and article["price"] < max_price,
        articles)
    )
    ...
    for article in articles_worth:
        ...
        if ("isFoil" in article and article["isFoil"] is True):
            ...
            if (is_price_deal(article["price"], price_table)):
